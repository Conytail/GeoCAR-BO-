...
ggsave(here::here('..','figs','compiled','figS1.newdraft.pdf'), supp_S1_1,
       height=7.5, width=8.25, useDingbats=FALSE)
...
## FigS2: compare abundance and length to expansion and proliferation
...
d3_ctv <- ggplot(car_len_abund_comparisons) + 
  ...
  labs(
    ...
    subtitle='Init. Lib. Abund. vs d3 Prolif. Score')
...
d3_ctv_len <- ggplot(car_len_abund_comparisons) + 
  ...
  labs(
    ...
    subtitle='Domain Length vs d3 Prolif. Score')
...
supp_S2_len_abund_vs_outcome <- len_abund_vs_outcome &
  theme_minimal() + 
  theme(panel.border=element_rect(fill=NA),
        panel.grid = element_blank())
...
cd69_panel <- ggplot(cd69.pct.pos) + 
  ...
  facet_wrap(~ t.type, scales='free_x', nrow=2) +
...
supp_fig_2 <- plot_grid(
  plot_grid(
    ...
    plot_grid(
      supp_S2_2_cd69_panel, supp_S2_5_interbin_volcano,
      nrow=1, labels=c('B','C')),
    plot_grid(
      supp_S2_4_pos_neg, supp_S2_len_abund_vs_outcome,
      nrow=1, labels=c('D','E')),
    ncol=1,
    rel_heights=c(0.8,1.2,1.8),
    labels=c('A','','C','')))
